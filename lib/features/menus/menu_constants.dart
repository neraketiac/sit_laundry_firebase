...
import 'package:laundry_firebase/features/menus/menu_receipt.dart';
import 'package:laundry_firebase/features/menus/menu_select_customer.dart';
import 'package:laundry_firebase/features/menus/menu_sub_main.dart';
...
var myAppBarX = AppBar(
    ...
    actions: [
      ...
      Badge(
        ...
        child: IconButton(
            onPressed: () {}, icon: const Icon(Icons.no_accounts_rounded)),
      ),
    ]);
          ...
          child: IconButton(
              onPressed: () {
                Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) => const MySelectCustomer()));
              },
              icon: Icon(userId > 0
          ...
          onTap: () {
            if (currentPage != "sales") {
              Navigator.push(context,
                  MaterialPageRoute(builder: (context) => const MyMenuMain()));
            } else {
          ...
          onTap: () {
            if (currentPage != "receipt") {
              Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) => const MyMenuReceipt()));
            } else {
              Navigator.pop(context);
            }
            currentPage = "receipt";
    ...
    child: InkWell(
      onTap: () {
        ...
        Navigator.of(context).push(
            MaterialPageRoute(builder: (context) => MySubMenu(id, price)));
      },
      child: Ink.image(
...
Future<void> addMenuDataX(BuildContext context) async {
  CollectionReference collRef =
      FirebaseFirestore.instance.collection('$sCollection/$fsKey/$fsKey');
  collRef
      .add({
        'sales_id': 123,
        'sales_name': 'test',
